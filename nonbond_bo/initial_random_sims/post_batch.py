from bayopt_util import load_bo, pbd
import sys
import os
import numpy as np
...
def get_score(dirname, loss = 'discrete'):
    obtained = []
    engs = os.path.listdir(dirname)
    for i, eng in enumerate(engs):
        filename = os.path.join(dirname, eng, 'vary_epshp', 'post_prod.lammpstrj')
        ...
        np.savetxt(os.path.join(dirname, eng, 'score_arr.txt'), [score])
        print(filename, score)
        obtained.append(score)
 
    obtained = np.array(obtained)

    test_scores = obtained.copy()    
