...
if __name__ == '__main__':
    ...
    if len(sys.argv) == 7:
        ...
        eps_00 = 0.10
        eps_11 = 0.15
        eps_22 = np.round(float(sys.argv[4]), num_dig) #0.5#0.5 #0.4
        eps_33 = np.round(float(sys.argv[5]), num_dig)
        frac = np.round(float(sys.argv[6]), num_dig)
        
        save_dir = f'batch_{batch_no}/test_{test_id}/eng_{param_id}'
    else:
        raise ValueError("Not 7 params")
    ...
    eps00 = eps_00#0.1#0.1
    eps11 = eps_11
    eps22= eps_22
    eps33 = eps_33#0.35#0.35
    eps_frac = frac
    ...
    json_dict = {var: str(globals()[var]) for var in json_vars if var in globals()}
    ...
    replace_vals = [temp,
                    ...
                    eps_33,
                    frac,
                    1, #exp not needed since not interpolating
                    1, #exp not needed since not interpolating
                    halfbox,
