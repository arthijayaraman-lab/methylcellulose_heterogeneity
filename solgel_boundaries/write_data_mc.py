...
def interp(temp, high, low, n, neg = True, plot = False):
...
def interpolate_engs(all_engs, exp, temp):
    temps = [298, 348]
    ...
    for bounds in all_engs.T:
        low = bounds[0]
        high = bounds[1]
        ret.append(interp(temp, high, low, exp))
    ret = np.array(ret)
    print(f'At {temp}K', ret, sep = '\n')
    return ret

def get_inputs():
    json_files = ['bound_params.json',
                  'chain_params.json',
                  'eng_params.json']
    with open('bound_params.json', 'r') as f:
        bound_params = json.load(f)
    with open('chain_params.json', 'r') as f:
        chain_params = json.load(f)
    with open('eng_params.json', 'r') as f:
        eng_params = json.load(f)
    
    return bound_params, chain_params, eng_params

bound_params, chain_params, eng_params = get_inputs()
...
save_dir = f'batch_{batch_no}/eng_{param_id}'

cell_eps_300 = eng_params['00_298']
cell_eps_348 = eng_params['00_348']
eps_11_300 = eng_params['11_298']
eps_11_348= eng_params['11_348']
eps_22_300 = eng_params['22_298']
eps_22_348 = eng_params['22_348']
mgf_eps_300 = eng_params['33_298']
mgf_eps_348 = eng_params['33_348']
exp = eng_params['exp']
frac_300= eng_params['frac_298']
frac_348 = eng_params['frac_348']
stretch = eng_params['stretch']
ramp = False
...
DS = float(sys.argv[4])
random_heterogeneity = float(sys.argv[5])
degree_of_polymerization = int(round(float(sys.argv[6])))
number_of_chains = int(round(float(sys.argv[7])))
wt_fraction = float(sys.argv[8])


if 'Hs' in bound_params.keys() or 'Hs' in chain_params.keys():
    uniform = False
if 'Hc' in bound_params.keys() or 'Hc' in chain_params.keys():
    uniform = True   
...
if __name__ == '__main__':
    ...
    z_spacing = 5.1 #inital spacing between backbone beads
