...
sys.path.append('../heterogeneities')
...
eng_params = get_inputs()

trial = int(sys.argv[1]) #1
ht = sys.argv[2] #'s' #'s'
h = float(sys.argv[3]) #1
ds = float(sys.argv[4]) #1.8

save_dir = f'sims/1000_100mers/trial_{trial}/ds{ds}_H{ht}{h}'
...
temp = 348
DS = ds
random_heterogeneity = h#float(sys.argv[5])
degree_of_polymerization = 100#int(round(float(sys.argv[6])))
number_of_chains = 100#int(round(float(sys.argv[7])))
wt_fraction = 0.02 #float(sys.argv[8])
...
#         spl = line.split()
#         if spl[1] == '2':
#             ids.append(int(spl[4])-1) #-1 for index
#     types = all_types[ids]
#     return types   
